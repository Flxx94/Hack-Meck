...
- Würfel-Upgrade (19.09.2026, nur `public/*`): Material (Verlaufs-Gradient + `::before`-Oberlicht, 72-%-Pips mit Highlight), Selected = weiß + angehoben + grüner Ring (`.set-group` grün statt grau), Hover `translateY(-5px) scale(1.03)`, Press `scale(.9)`; Wurf-Keyframe `diceroll` in 3 Phasen (Anheben → Tumble → Squash-Landung) mit Stagger 70 ms + `--rdur` 540–680 ms + `--rd`-Richtung pro Würfel; Wurm landet mit einmaligem Grün-Ring (`wormland`); optimistische Anhebung (`.throwing` + `rollStart`) beim Klick bis Server antwortet.
- Sound-Ausbau (WebAudio-Synth, keine Assets): `rollStart` (Whoosh), Lande-Ticks pro Würfel (`tick(i)` trocken/leise, `wormTick` + Chime bei Wurm, gestaffelt per `scheduleLandingTicks`, Sequenz-Guard `S.rollSeq`, Reduced-Motion → 1 Tick), `pickWorm`-Special, `cardSlide` bei Abflug + `cardPlace` bei Landung (Once-Guard). 3-Stufen-Lautstärke am Sound-Button (🔊→🔉→🔇, `heckmeck-volume`, Master-Gain, migriert altes `heckmeck-muted`); Start weiter erst nach Nutzer-Geste.
